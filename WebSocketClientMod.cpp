...
void WebSocketClientMod::setAuthorizationHeader(String header) {
	this->authorizationHeader = header;
}

String WebSocketClientMod::generateKey() {
	String key = "";
	for (int i = 0; i < 23; ++i) {
		int r = random(0, 3);
		if (r == 0)
...
void WebSocketClientMod::write(uint8_t data) {
    if (client->connected())
        client->write(data);
}

void WebSocketClientMod::write(const char *data) {
    if (client->connected())
        client->write(data);
}

//String WebSocketClientMod::generateKey2() {
//	String key = "";
//	// TODO generate valid key... @see libraries mWebSocket
//	return key;
//}


bool WebSocketClientMod::connect(String host, String path, int port) {
...
bool WebSocketClientMod::isConnected() {
	return this->websocketEstablished && client->connected();
}

void WebSocketClientMod::disconnect() {
	client->stop();
    this->websocketEstablished = false;
}

void WebSocketClientMod::send(const String& str) {
...
int WebSocketClientMod::timedRead() {
...
bool WebSocketClientMod::getMessage(String& message) {
